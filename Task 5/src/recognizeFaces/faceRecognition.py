...
class FaceRecognition():
    ...
    def loadDataSet(self):
        ...
        self.all_img = np.zeros((self.N_IMGS,self.shape[0],self.shape[1]),dtype='float64')
        self.names = list()
        i = 0
        ...
        for folder in glob.glob(self.dataSetPath + '/*'):
            ...
            for image in glob.glob(folder +'/*'):
                ...
                resized_image =cv2.resize(read_image,(self.shape[1],self.shape[0]))
    ...
    def meanImgs(self):
        A = np.resize(self.all_img,(self.N_IMGS,self.shape[0]*self.shape[1]))#Creating a matrix of n^2 x m
        self.mean_vector = np.sum(A,axis=0,dtype='float64')/self.N_IMGS
        self.mean_matrix = np.tile(self.mean_vector,(self.N_IMGS,1))#Calculating mean for all the 400 images
    ...
    def classifyImg(self,input:np.ndarray):
        ...
        mean_sub_testimg=np.reshape(input,(input.shape[0]*input.shape[1]))- self.mean_vector
...
if __name__ == '__main__':
    ...
    def FaceRecognitionWrapper(input:np.ndarray):
        FaceRecognizer = FaceRecognition('../../imgs/archive')
        
        try:
            result = FaceRecognizer.classifyImg(input)
        except:
            print("not found")
